...
function buildOrgJqFilter(options = {}) {
...
async function fetchOrgRepos(org, options = {}) {
  validateOrgName(org);
  let stdout;
  try {
...
async function fetchRunsForPeriod(repo, start, end) {
  try {
...
function getMonthPeriods(since, until) {
  const periods = [];
  ...
  while (current <= endDate) {
    ...
    const periodStart = current.getTime() === startDate.getTime() ? since : `${year}-${String(month + 1).padStart(2, "0")}-01`;
...
async function fetchRepoRuns(repo, since, until) {
...
async function fetchMultiRepoRuns(repos, since, until) {
...
async function resolveRepos(org, repos, options = {}) {
  const orgFilter = {
...
function formatResolveLog(result, org) {
...
function getDurationMinutes(startedAt, updatedAt) {
  const start = new Date(startedAt).getTime();
  const end = new Date(updatedAt).getTime();
  return Math.max(0, (end - start) / 6e4);
}
function getOrCreateUser(byActor, actor, repo) {
...
function computeTotals(users, months) {
  return {
...
function aggregate(runs, repos, since, until, sortBy) {
...
function groupByActor(data, sortBy = "minutes") {
  ...
  const users = [...grouped.entries()].map(([actor, stats]) => ({
    actor,
    repo: GROUPED_REPO,
...
function formatFetchSummary(results) {
  ...
  const maxLen = active.length > 0 ? Math.max(...active.map((r) => r.repo.length)) : 0;
...
var rightAligned = (content) => ({ content, hAlign: "right" });
function renderTable(data) {
  ...
  const head = [
    ...
    source_default.bold("Total min"),
    source_default.bold("Hours"),
    source_default.bold("Runs"),
    ...months.map((m) => source_default.bold(formatMonthLabel(m)))
  ];
  ...
  for (const user of users) {
    table.push([
      ...
      ...months.map(
        (m) => rightAligned(Math.round(user.monthlyMinutes[m] ?? 0).toLocaleString())
      )
    ]);
  }
  table.push([
    ...
    ...showRepo ? [""] : [],
    rightAligned(source_default.bold(Math.round(totals.minutes).toLocaleString())),
    rightAligned(source_default.bold((totals.minutes / 60).toFixed(1))),
    rightAligned(source_default.bold(totals.runs.toLocaleString())),
    ...months.map(
...
function renderMarkdown(data, filePath) {
  ...
  lines.push("");
  lines.push(
    repos.length > 1 ? `**${repos.length} repositories** | ${data.since} to ${data.until}` : `**${repos[0]}** | ${data.since} to ${data.until}`
  );
  lines.push("");
  const headers = [
    "Developer",
...
`
  ...
  const results = repos.length === 1 ? [await fetchRepoRuns(repos[0], since, until)] : await fetchMultiRepoRuns(repos, since, until);
...
).option("--until <date>", "end date YYYY-MM-DD (default: today)").addOption(
  new Option("--format <type>", "output format").choices(["table", "csv", "json", "markdown"]).default("table")
).addOption(
...
).option("--pr <number>", "show CI cost for a specific pull request", (val) => {
  const n = Number(val);
